...
void AudioManager::createSoundCache()
{
    ...
    if (!m_noAudio)
    {
        if (Mainapp::getInstance()->isAudioThread())
        {
            QStringList searchFolders = Filesupport::createSearchPath("sounds/");
...
void AudioManager::SlotAddMusic(QString file, qint64 startPointMs, qint64 endPointMs)
{
    ...
    if (!m_noAudio)
    {
        QString currentPath = Filesupport::locateResource(file);
        if (!QFile::exists(currentPath) && (file.endsWith(".wav") || file.endsWith(".mp3")))
        {
            // Try using OGG instead of mp3/wav if it is available.
            currentPath = Filesupport::locateResource(file.first(file.length() - 4) + ".ogg");
        }
