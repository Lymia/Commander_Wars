...
void Terrain::setMapForExtending(GameMap * newMap)
{
    ...
    if (m_pBaseTerrain.get() != nullptr)
    {
        m_pBaseTerrain->setMapForExtending(newMap);
    }
    if (m_Building.get() != nullptr)
    {
        m_Building->setMapForExtending(newMap);
    }
    if (m_Unit.get() != nullptr)
    {
        spUnit pUnit = m_Unit;
        setUnit(spUnit());
        if (pUnit->setMapForExtending(newMap))
        {
            setUnit(pUnit);
        }
    }
...
void Terrain::loadBaseSprite(const QString & spriteID, qint32 frameTime, qint32 startFrame, qint32 endFrame)
{
    ...
    else if (customSpriteExists())
    {
        ...
        m_terrainSpriteName = spriteID;
        QImage img;
        QString imgPath = Filesupport::locateResource(m_terrainSpriteName);
...
bool Terrain::customSpriteExists() const
{
    ...
    return pAnim != nullptr ||
                    QFile::exists(Filesupport::locateResource(m_terrainSpriteName));
}
